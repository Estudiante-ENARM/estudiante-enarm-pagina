...
logoArea.addEventListener('click', () => {
  // keep previous toggle behavior for desktop users
  sidebar.classList.toggle('closed');
});

// --- Mobile drawer handlers (added) ---
// NOTE: We intentionally DO NOT add the '.open' class when opening on mobile.
// The '.open' class is used for desktop collapsed/expanded behavior and caused
// the sidebar to remain visible (pushing content) when it shouldn't.
function openMobileDrawer(){
  sidebar.classList.add('open-mobile');
  // removed: sidebar.classList.add('open');
...
function closeMobileDrawer(){
  ...
  // ensure 'open' (desktop state) is removed on mobile close to avoid layout issues
...
saveWhatsappBtn.addEventListener('click', ()=> saveSocialLinksObject({ whatsapp: inputWhatsapp.value.trim() }));
saveTiktokBtn.addEventListener('click', ()=> saveSocialLinksObject({ tiktok: inputTiktok.value.trim() }));
saveInstagramBtn.addEventListener('click', ()=> saveSocialLinksObject({ instagram: inputInstagram.value.trim() }));
saveTelegramBtn.addEventListener('click', ()=> saveSocialLinksObject({ telegram: inputTelegram.value.trim() }));
